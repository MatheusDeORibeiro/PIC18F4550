...
**  Versão: v0.1											 *
**  Data: 12/03/2011                                                                                     *
**  Modificação: 02/07/2021 - Ajuste na biblioteca para ela se adequar ao compilador xc8                 *
*********************************************************************************************************/

//#include<delays.h> //Adiciona a biblioteca de delay.; Biblioteca não mais necessária para o compilador xc8
#include <xc.h> //Adiciona a biblioteda de funções específicas do dispositivo
/*
#ifndef Fosc
...
void gera_atraso_us(unsigned int atraso)//adaptado para Fosc de 20MHz
{
  do 
  {
    _delay(1); //Gera um delay de 1 ciclo de máquina; Delay1TCY (); //Gera um delay de 1 ciclo de máquina.
    _delay(1); //Gera um delay de 1 ciclo de máquina; Delay1TCY (); //Gera um delay de 1 ciclo de máquina.
    _delay(1); //Gera um delay de 1 ciclo de máquina; Delay1TCY (); //Gera um delay de 1 ciclo de máquina.
    _delay(1); //Gera um delay de 1 ciclo de máquina; Delay1TCY (); //Gera um delay de 1 ciclo de máquina.
    _delay(1); //Gera um delay de 1 ciclo de máquina; Delay1TCY (); //Gera um delay de 1 ciclo de máquina.
...
RW = (unsigned char)escrita_leitura; //Informa se é escrita ou leitura.
